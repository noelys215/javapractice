...
public class PokerHand {
    ...
    public void evalHand() {
        ...
        List<String> dupeFaceCards = new ArrayList<>();
        faceList.forEach(face -> {
            if (!dupeFaceCards.contains(face) && Collections.frequency(faceList, face) > 1)
                dupeFaceCards.add(face);
        });
        for (String duplicate : dupeFaceCards) {
